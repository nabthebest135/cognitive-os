...
export class UniversalActionExecutor {
  async executeUniversalAction(intent: Intent, userInput: string): Promise<ActionResult> {
    // If we have AI response, use that
    if (intent.aiResponse) {
      const filename = `ai_response_${Date.now()}.md`;
      const content = `# AI Response\n\n## Your Question\n"${userInput}"\n\n## Answer\n${intent.aiResponse}\n\n---\n*Generated by Cognitive OS AI*`;
      
      this.downloadFile(content, filename, 'text/markdown', 'AI-generated response to your question');
      
      return {
        success: true,
        message: `✅ AI response downloaded as ${filename}`
      };
    }
